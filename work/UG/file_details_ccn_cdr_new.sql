start transaction;
delete from  audit.files_details where file_date=yyyymmdd and feed_name=upper('CCN_CDR');
insert into audit.files_details

select  distinct base_file_name,inst,nodeid,cast(seq as int) seq,
cast(linecount as int),fulldate,cast(dt as int),upper('CCN_CDR') from
(
...
where tbl_dt between ydate and vday_2  and file_name like '%yyyymmdd%' and file_name not like '%OCC%') ;
commit;



start transaction;
delete from  audit.missing_sequance where file_date=yyyymmdd and feed_name=upper('CCN_CDR');
insert into audit.missing_sequance
select  node1,instance1 ,sequence2+1 ,sequence1-1,filedate,upper('CCN_CDR')
...
FROM   audit.files_details where feed_name=upper('CCN_CDR') and file_date>=ydate and file_date<=vday_2
order by SEQUENCE
)) a left outer join
(
select rank() over ( partition by node_id,instance order by SEQUENCE asc) num, sequence,file_name,node_id,instance,fulldate,file_date
